...
const jsonImporter = async (db: DB, fileName: string, collectionName: string) => {
  const pathName = path.join(__dirname, `./resource/${fileName}`);
  const data = fs.readFileSync(pathName, "utf-8");
  const jsonData: Array<Record<string, any>> = JSON.parse(data)
  const collection = db.collection(collectionName);
  await collection.dropCollection()
  const newCollection = db.collection(collectionName);
  await newCollection.insertMany(jsonData)
}

export {csvImporter, jsonImporter}